...
async function adminAuth(req, res, next) {
  const shop = req.query.shop || req.headers['x-shopify-shop-domain'] || (req.session && req.session.shop);
  if (!shop) return res.status(400).json({ error: 'Shop required' });
  if (process.env.NODE_ENV === 'development' && !req.query.hmac && !req.query.signature) {
    req.shop = shop; return next();
  }
  if (req.query.hmac && verifyHmac(req.query, SHOPIFY_API_SECRET)) { req.shop = shop; return next(); }
...
app.get('/api/auth/callback', async (req, res, next) => {
  try {
    await shopify.auth.callback()(req, res, (err) => {
      if (err) { error('OAuth callback error: ' + (err.message || err)); return res.status(500).send('OAuth callback failed: ' + (err.message || err)); }
      next();
    });
...
db.init().then(() => {
  app.listen(PORT, () => log(`EU Commerce Suite listening on port ${PORT}`));
}).catch(err => {
